...
<body>
  ...
  <nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      ...
      <!--Modal: Login / Register Form-->
      <div>
        <a style="color:gray; text-decoration: none; margin-right:10px;" href="pregled-narudzbi.php">Pregled
          narudžbi</a>
  ...
  $sql = "SELECT 
    ...
    narudzbe.vrijeme_narudzbe, 
    stavkejelovnika.Naziv, 
    narudzbastavke.quantity 
    FROM 
      narudzbe
    JOIN 
      korisnik ON narudzbe.IDkorisnika = korisnik.ID
    JOIN 
      narudzbastavke ON narudzbe.ID = narudzbastavke.id_narudzbe
    JOIN 
      stavkejelovnika ON narudzbastavke.id_stavke = stavkejelovnika.ID
    ORDER BY 
      narudzbe.vrijeme_narudzbe DESC, 
      narudzbe.ID DESC;";
    ...
    while ($row = $result->fetch_assoc()) {
      if ($currentOrderId !== $row['ID']) {
        if ($currentOrderId !== null) {
          // End the current order's details div and list
          echo '</ul></div>';
        }

        // New order details
        $formattedDate = (new DateTime($row["vrijeme_narudzbe"]))->format('d.m.Y H:i');
        echo '<div class="mb-4 border p-3">';
        echo '<h5>' . $row["KorisnickoIme"] . ' - ' . $formattedDate . '</h5>';
      ...
      echo $row["Naziv"];
      echo '<span class="badge badge-warning badge-pill">' . $row["quantity"] . '</span>';
      echo '</li>';
    }
    // End the last order's details div and list
    echo '</ul></div>';
  } else {
    echo "Nema narudžbi.";
  }
